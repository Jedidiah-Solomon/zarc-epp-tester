...
class EPPTCPTransport{
   ...
   function __construct($ip="127.0.0.1",$port=3121,$clientpemfile=false,$ssl=true) {
     if ($ssl) {
...
$epp = new EPPTCPTransport("regphase3.dnservices.co.za",3121);
print $epp->greeting."\n\n";
print "------------------\n";
for ($i=1;$i<$argc;$i++){
  print "------------------".$argv[$i]."---------------------\n";
  print $epp->chat(file_get_contents($argv[$i]));
}
$epp->close();
